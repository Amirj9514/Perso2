...
@Component({
 ...
 imports: [
  ...
  templateUrl: './behorde.component.html',
  styleUrl: './behorde.component.scss'
})
export class BehordeComponent {
  ...
  getEmployeeDetails(): void {
    ...
    this.sharedS.sendGetRequest(url).subscribe({
      next: (resp) => {
        ...
        if (resp.status === 200) {
          this.products = resp?.body?.authorities ?? [];
        } else {
          this.products = [];
        }
      },
  ...
  onAccept(): void {
    ...
    this.sharedS.sendDeleteRequest(`authorities/${this.selectedRow.id}`).subscribe({
      next: (response) => {
