...
@Component({
  ...
  templateUrl: './s81a.component.html',
  styleUrl: './s81a.component.scss',
})
export class S81aComponent {
  ...
  constructor(
    ...
    const formJson = this.formsJsonS.getTab7FormJson();
    this.createForm(formJson);
    this.tab1FormJson = this.formsJsonS.getTab7FormJson();
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['applicant']?.currentValue) {
      this.tab1SaveValue = this.applicant.tab_7;
  ...
  onFormSubmit(formValue: any) {
    const apiParams = {
      ...
      tab_7: JSON.stringify(formValue),
    };
