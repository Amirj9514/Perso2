...
@Component({
  ...
  templateUrl: './urkunde.component.html',
  styleUrl: './urkunde.component.scss'
})
export class UrkundeComponent {
  @Input() applicant: any;
  ...
  constructor(
    ...
    const formJson = this.formsJsonS.getTab8FormJson();
    this.createForm(formJson);
    this.tab1FormJson = this.formsJsonS.getTab8FormJson();
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['applicant'].currentValue) {
      this.tab1SaveValue = this.applicant.tab_8;
  ...
  onFormSubmit(formValue: any) {
    const apiParams = {
      ...
      tab_8: JSON.stringify(formValue),
    };
